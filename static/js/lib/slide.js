...
//  根据循环与否做些操作
    ...
    //生成下标
    var _html = '<li class="active"></li>'
    for (var i = 1; i < size; i++) {
        _html += '<li></li>';
    }
    $($page).append(_html).find('li').on('click', function() {
...
Slide.prototype._doPlay=function(){
    ...
    if(ops.$page){
        ops.$page.find("li").eq(self.index).addClass("active").siblings().removeClass("active");
    }
};
...
Slide.prototype._end=function(e){
    ...
    ops.element.removeEventListener(ops.events.end,self,false);
...
Slide.prototype._autoRun=function(s){
    var self=s;
    var fn=function(scope){
        return setTimeout(function(){
...
Slide.prototype.toPre=function(){
    var self=this;
    self.index--;
    self._doPlay();
};
